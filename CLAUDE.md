...
| 좁은 줄의 보조 라벨 (예외) | 12px | `var(--dia-text-2xs)` — 값 옆에 붙는 짧은 라벨 한정 |
| 절대 최솟값 | **12px** | `var(--dia-text-2xs)` 예외 자리에만 |
| 12px 미만 | **금지** | 어떤 경우에도 불허 |
| 본문/버튼/CTA | 14px 이상 | 읽을 거리는 줄이지 않는다 |

- 토큰 `--dia-text-xs`, `--dia-text-sm` 최솟값 = **14px**
- 토큰 `--dia-text-meta` = **13px** (예외 한정: 타임스탬프, 배지, 태그, 칩, 메타데이터, 카운트, 단위, 인디케이터)
- 토큰 `--dia-text-2xs` = **12px** (예외 한정: 값과 짝을 이루는 짧은 라벨 — 「교대받음 홍운기」의 «교대받음»처럼
  옆의 값이 14px 이상이라 뜻이 흐려지지 않는 자리. 혼자 서는 글에는 쓰지 않는다)
- 핵심 숫자(교번/시간) 24-32px
